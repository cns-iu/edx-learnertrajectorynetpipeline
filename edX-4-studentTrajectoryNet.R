...
#               as JSON graphs and as a node and edge list CSV files that can be 
#               used with analysis and visualization.
# 
# File input stack: 
#               1) Course Structure data set - generated by A.B. python script
#                  (need to convert from Python to R)
#               2) A folder containing one or more "*.csv" event log file(s) -
#                  generated by script, "edX-2-studentLogFormatter.R", 
#                  using processed edX logs
#  
# Package dependencies: magrittr, zoo, stringr, plyr, igraph, tcltk, jsonlite, 
#                       and a fork of graph-importer-R functions
#
# Changelog:
#   2017.11.02. Version 1: Initial Code - environment, look up table
#   2017.11.03. Fix look-up table; create edge list and node list - initial network created
#   2017.11.06. Update JSON export function; update network statistics; load package reqs; 
#               export files as CSV and JSON
#   2017.11.09. Updated node list creation to add indicators of course hierarchy: third level 
#               module sequence groups and first level course chapters/week groups; adjusted
#               the maximum time for event from 150 minutes to 60; remove edge type from 
#               export, as type of transition is not captured by this field; convert node 
#               and edge factor and time variables to characters and numeric variable types 
#               to remove warnings given by igraph creation function.
...
#               currently in course structure. Update edge list loop to identify and remove  
#               low level modules that are not in course structure.
...
#               "edX-2-studentLogFormatter.R" script, e.g. moved module identifier look-up
#               routine, calculation of duration of time at event, and removal of modules
#               from event log that are not in final course structure. 
#               Updated function courseStrNodes to include mod_hex_id field instead of full
#               module.key identifier, added courseID field to generic node list.
...
courseStrNodes <- function(courseStr,nc=FALSE){
  if(missing(courseStr)){stop("No Course Structure provided")
    ...
    if(nc==FALSE){
      ...
      nodes <- courseStr[courseStr$treelevel==4,c(1,2,4,5,8,9)]
      ...
      for(i in 1:nrow(nodes)){
        nodes[i,]$L2 <- as.character(courseStr[courseStr$mod_hex_id==as.character(nodes[i,]$L3),]$parentID)
        nodes[i,]$L2label <- as.character(courseStr[courseStr$mod_hex_id==nodes[i,]$L2,]$name)
        nodes[i,]$L1 <- as.character(courseStr[courseStr$mod_hex_id==as.character(nodes[i,]$L2),]$parentID)
      ...
      parentID <- rep("course",3)
      modparent_childlevel <- paste(parentID,childlevel,sep="/")
      d <- data.frame(mod_hex_id,courseID,child.full,type,name,treelevel,childlevel,order,parentID,modparent_childlevel)
      #Add three new rows to course structure
      courseStr <- rbind(courseStr,d)
      rm(d,mod_hex_id,courseID,child.full,type,name,treelevel,childlevel,order,parentID,modparent_childlevel)
...
#path for saving user lists
#path_users = tclvalue(tkchooseDirectory())
path_users <- c("C:/Users/mginda/Desktop/Boeing/edx/MITProfessionalX_SysEngxB1_3T2016/userlists")

#path_data = tclvalue(tkchooseDirectory())
path_data <- c("C:/Users/mginda/Desktop/Boeing/edx/MITProfessionalX_SysEngxB1_3T2016/studentevents/results")

#path_output = tclvalue(tkchooseDirectory())
path_out <- c("C:/Users/mginda/Desktop/Boeing/edx/MITProfessionalX_SysEngxB1_3T2016/networks")

##Create data processing output sub-directories
...
#course structure 
structure <- read.csv("C:/Users/mginda/Desktop/Boeing/edx/MITProfessionalX_SysEngxB1_3T2016/module-lookup-result.csv",header=T)
structure$courseID <- sapply(strsplit(as.character(structure$courseID),":"),"[",2)
structure$courseID <- substr(structure$courseID,1,nchar(structure$courseID)-1)

###Modification of course structure for accurate node representation for courses
nodes <- courseStrNodes(courseStr=structure, nc=F)
rm(structure)

##Load list of users 
d <- read.csv(paste0(path_users,"/","EdX-SysEngB1+3T2016-user_id-events.csv"))
names(d) <- "id"
logFilePaths <- paste0(path_data,"/",d$id,".csv")
rm(d)
...
for(i in 1:numLogs){
    ...
    if(nrow(data)>1){
      ...
      #Removes self loops, and reorganizes the columns
      edges <- edges[!(edges$to==edges$from),c(2,6,3:5,7,9,10,8,11,12,1)]
      str(edges)
      if(nrow(edges)>0){
        ###Node Statistics
        ...
        rm(ep,er)
        names(et)[1] <- names(es)[1] <- c("mod_hex_id")
        ...
        write.csv(nodes_t,paste0(path_out,"/",subDir[1],"/",sid,"-nodes.csv"),row.names=F)
        str(nodes_t)
        ...
        write.csv(edges,paste0(path_out,"/",subDir[2],"/",sid,"-edges.csv"),row.names=F)
        ...
        g$name <- sid
        exportGraph(g,filename=paste0(path_out,"/",nodes$courseID[1],"-",sid,".json"))
      } else {
        write.csv(data,paste0(path_out,"/",subDir[4],"/",sid,".csv"),row.names=F)
      }
    } else {
      write.csv(data,paste0(path_out,"/",subDir[3],"/",sid,".csv"),row.names=F)
    }
}
...
write.csv(x=users[,1], file=paste0(path_users,"/","EdX-SysEngB1+3T2016-user_id-hasNet.csv"),row.names = F)

users <- list.files(path=paste0(path_out,"/",subDir[3]),pattern=".csv")
users <- data.frame(do.call('rbind',strsplit(users,"\\.")))
names(users) <- c("userID","v")
write.csv(x=users[,1], file=paste0(path_users,"/","EdX-SysEngB1+3T2016-user_id-oneEvent.csv"),row.names = F)

users <- list.files(path=paste0(path_out,"/",subDir[4]),pattern=".csv")
users <- data.frame(do.call('rbind',strsplit(users,"\\.")))
names(users) <- c("userID","v")
write.csv(x=users[,1], file=paste0(path_users,"/","EdX-SysEngB1+3T2016-user_id-selfLoops.csv"),row.names = F)
