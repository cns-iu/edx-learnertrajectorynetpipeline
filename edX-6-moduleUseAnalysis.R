...
# 
# Description:  This script processes sets of student's edX course trajectory networks 
#               (single period) to create an aggregated course trajectory network for
#               a group of students. The script relieson individual student's course
#               transition networks, which represent their movement between EdX block
#               learning modules and use of modules based on processed event logs from 
#               an EdX course.
#
...
#               - {userID}-nodes.csv and {userID}-edges.csv 
#               - generated by script by "edX-4-studentTrajectoryNet.R"
#
# Output files:
#            1)
#
...
write.csv(mods,file=paste0(path_output,courseID,groupID,"+ModuleUseStats-NodeList.csv"), row.names = F)
rm(nodeFileList,rec)
