...
stdData <- read.csv(list.files(full.names = TRUE, recursive = T, 
                               path = paste0(path_data,"/",courseID,"/state/"),
                               pattern = "auth_user-prod-analytics"),sep="\t")[,c(1,10,12:15)]

#Update courseID for exporting file name
courseID <- gsub("\\_","-",courseID)
...
#Creats list paths to laod individual student event logs
...
##Save seperate output data files
#Video Data
